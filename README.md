...
- [ ] Сделать обёртку для сборки. (На самом деле взять готовую и адаптировать под нас)



## Подготовка к сборке контейнера
Подразумевается, что у вас установлена Ubuntu 24 любым способом: Bare Metal, VPS, WSL. 

Внимание! Для работы CUDA в контейнере версия драйвера хоста должна быть **новее**, чем версия CUDA в контейнере. Но с Ubuntu 18 условие точно выполнится.
...
# В соседней консоли можно также зайти в уже запущеный контейнер, т.к. для ROS обычно одного терминала мало.
...
**Проверяйте пути --volume=~/docker/ubuntu18/ros_ws:/ros_ws**
...
# Включаем графику для локального пользователя, состоящего в группе docker. Потенциально опасная настройка, рекомендуется после использования контейнера выполнить xhost -local:docker
